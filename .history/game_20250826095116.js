    ...
    function drawVeggie(v) {
        ...
        switch (v.type) {
            case "carrot": {
                ...
                const grad = ctx.createLinearGradient(-bodyW * 0.3, -bodyH, bodyW, bodyH);
                ...
                ctx.quadraticCurveTo(-bodyW * 0.2, bodyH * 0.65, -bodyW * 0.35, bodyH * 0.6);
                ctx.quadraticCurveTo(-bodyW * 0.55, bodyH * 0.15, -bodyW * 0.45, -bodyH * 0.6);
                ctx.closePath();
                ctx.fill();
                // subtle outline
                ...
                for (let i = -2; i <= 2; i++) {
                    const t = i / 5;
                    ctx.beginPath();
                    ctx.ellipse(0, t * bodyH * 0.8, bodyW * (0.7 - Math.abs(t) * 0.4), bodyH * 0.06, 0, 0, Math.PI * 2);
                    ctx.stroke();
                }
                // highlight
                ctx.fillStyle = "rgba(255,255,255,0.18)";
                ctx.beginPath();
                ctx.ellipse(-bodyW * 0.1, -bodyH * 0.25, bodyW * 0.12, bodyH * 0.18, -Math.PI / 8, 0, Math.PI * 2);
                ctx.fill();
                // leaves (more defined)
