(() => {
  ...
  function createVeggie() {
    ...
    const type = ["lettuce", "carrot", "cabbage", "pepper"][Math.floor(rand(0, 4))];
    const colorMap = {
      lettuce: "#86efac",
      carrot: "#fb923c",
      cabbage: "#a78bfa",
  ...
  function draw() {
    ...
    ctx.lineWidth = 2;
    ctx.strokeRect(WORLD_PADDING, WORLD_PADDING, w - WORLD_PADDING*2, h - WORLD_PADDING*2);
  ...
  function drawVeggie(v) {
    ...
    ctx.beginPath();
    ctx.ellipse(-v.radius*0.3, -v.radius*0.3, v.radius*0.2, v.radius*0.35, Math.PI/6, 0, Math.PI*2);
    ctx.fill();
    ctx.restore();
  }
  ...
  function drawSnail(s) {
    ...
    ctx.beginPath();
    ctx.arc(eyeX + s.radius * 0.05, -s.radius * 0.2, s.radius * 0.07, 0, Math.PI*2);
    ctx.arc(eyeX + s.radius * 0.05, s.radius * 0.2, s.radius * 0.07, 0, Math.PI*2);
    ctx.fill();
    ...
    const shellGrad = ctx.createRadialGradient(-shellR*0.2, -shellR*0.2, shellR*0.3, 0, 0, shellR);
