(() => {
    ...
    function updateHUD() {
        ...
        if (timebar) {
            const pct = Math.max(0, Math.min(1, state.timeLeft / GAME_DURATION_SECONDS));
            timebar.style.transform = `scaleX(${pct})`;
            // color shift near the end
            if (pct < 0.2) {
                timebar.style.background = 'linear-gradient(90deg, #ef4444, #f97316)';
            } else if (pct < 0.5) {
                timebar.style.background = 'linear-gradient(90deg, #f59e0b, #a3e635)';
            } else {
                timebar.style.background = 'linear-gradient(90deg, #22c55e, #a3e635)';
            }
        }
    }
